...
import com.example.anew.adjust.WeightedLeastSquares;
import com.example.anew.constellations.GnssConstellation;
import com.example.anew.constellations.GpsConstellation;
import com.example.anew.navifromftp.Coordinates;
import com.example.anew.navifromftp.RinexNavigationGalileo;
...
public class MainActivity extends AppCompatActivity {
    ...
    private Button bt_ftp, bt_start, bt_stop,bt_map;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bt_ftp.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
        ...
        bt_map=findViewById(R.id.newmap);
        bt_map.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                startActivity(new Intent(MainActivity.this, MapActivity.class).setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP));
            }
        });
        ...
        public void onLocationChanged(Location location) {
            if (isRecordStart && !isRecordStop) {
                if (location != null && !poseinitialized) {
                    ...
                    //**************************************************************//
                    textView7.setText(String.format("Latitude: %.6f\nLongitude: %.6f\nAltitude: %.2f", pose.getGeodeticLatitude(), pose.getGeodeticLongitude(), pose.getGeodeticHeight()));
                    //**************************************************************//
        ...
        public void onGnssMeasurementsReceived(GnssMeasurementsEvent eventArgs) {
            ...
            if (isRecordStart && !isRecordStop) {
                ...
                if(mGnssConstellation.getUsedConstellationSize()>=5)
                {
                    ...
                    Coordinates pose=mWeightedLeastSquares.calculatePose(mGnssConstellation);
                    //**************************************************************//
                    //textView8.setText(String.format("X: %.6f  Y: %.6f  Z: %.2f", spppose.getX(), spppose.getY(), spppose.getZ()));
                    textView8.setText(String.format("Latitude: %.6f  Longitude: %.6f", pose.getGeodeticLatitude(), pose.getGeodeticLongitude()));
                    // 创建 Intent 并发送广播
                    final String ACTION_SPP_UPDATE="com.example.SPP_UPDATE";
